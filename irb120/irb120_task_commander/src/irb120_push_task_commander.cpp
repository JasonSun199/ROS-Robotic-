//irb120_push_reactive_task_commander.cpp
//Jason Sun, hxs514
//this version is a variation on irb120_task_commander, with magic_object_finder server and can move object sideways.
...
string g_object_name("gear_part");  //Name of the object needed to be found in Gazebo!!!!!!
...
int main(int argc, char** argv) {
    ros::init(argc, argv, "push_task_commander"); // name of the node
    ...
    //xxxx  use the x and y coordinates of the gear part, but specify a higher z value to make the flange exactly above the gear part.
    ...
    //go to pose to the left of the gear part. Adjust the x and y coordinates to make sure the flange is nicely to the left of the gear part and then lower the flange by setting the z coordinate to 0
    flange_origin <<(g_perceived_object_pose.pose.position.x)+0.03, (g_perceived_object_pose.pose.position.y)-0.1, 0.006;
    ...
    //xxxxxxxxxxxxxxxxxx
    //Make the flange push the gear part sideways along the y axis:
    //  -maintain the x and z coordinates. change the y coordinates large in order to push it to the positive direction of the y axis.
    flange_origin << (g_perceived_object_pose.pose.position.x)+0.03, (g_perceived_object_pose.pose.position.y)+0.22, 0.006;
    goal_flange_affine.translation() = flange_origin;
    ROS_INFO_STREAM("move to flange origin= " << goal_flange_affine.translation().transpose() << endl);
...
//Make the flange raise up:
    //  -maintain the x and y coordinates. change the z coordinates large.
    flange_origin << (g_perceived_object_pose.pose.position.x)+0.03, (g_perceived_object_pose.pose.position.y)+0.22, 0.3;
    goal_flange_affine.translation() = flange_origin;
    ROS_INFO_STREAM("move to flange origin= " << goal_flange_affine.translation().transpose() << endl);
    g_q_vec_arm_Xd = optimal_path.back(); //start from the joint-space pose that ended the prior plan
    //convert move to an optimal joint-space path:
