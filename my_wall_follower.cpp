//Modified to make the robot's movement more smooth. 
#include <ros/ros.h>
...
//new front ping to find a wall. 
float g_radius_front; 
...
void laserCallback(const sensor_msgs::LaserScan& laser_scan) {
    ...
    g_radius_front = laser_scan.ranges[g_index_front]; 
...
ROS_INFO("Turning to the desired direction!"); 
twist_commander.publish(twist_cmd);
ros::spinOnce();
loop_timer.sleep();
 ...
 */
    ...
    //the robot will keep moving forward, until the front ping touch the wall
    ...
    while (ros::ok()) {
        ...
        if ((g_radius_min < WALL_FOLLOW_RADIUS) && (g_index_min_dist_ping < g_index_tangent_left)) {
            ...
            twist_commander.publish(twist_cmd);
            double dt = fabs(dtheta / YAW_RATE)- 0.09;
        ...
        twist_commander.publish(twist_cmd);
        while ((g_clearance_tan_test < (WALL_FOLLOW_RADIUS/cos(DANG_TANGENT_APPROX))) && (g_index_min_dist_ping >= g_index_tangent_left)) {
        ...
        if ((g_radius_min < WALL_FOLLOW_RADIUS) && (g_index_min_dist_ping < g_index_tangent_left)) {
            ...
            ROS_WARN("min ping dist = %f at index %d", g_radius_min, g_index_min_dist_ping);
        } else if (g_clearance_tan_test > (WALL_FOLLOW_RADIUS/cos(DANG_TANGENT_APPROX))) {
            ...
            while (g_clearance_tan_test > (WALL_FOLLOW_RADIUS/cos(DANG_TANGENT_APPROX))) {
